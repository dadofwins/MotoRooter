...
def span(start_m: float, end_m: float) -> RouteLeg:
    """A leg occupying its own stretch of road, so legs do not sit on top of each other."""
    points = tuple(north(start_m + (end_m - start_m) * index / 100) for index in range(101))
    return RouteLeg(
        geometry=points,
        ...
        surface_spans=(
            SurfaceSpan(
                surface=Surface.PAVED, distance_m=end_m - start_m, start_index=0, end_index=100
            ),
        ),
        provider="fake",
        intent=LegIntent.TWISTY_PAVED,
...
def beside(name: str, score: float, *, at_m: float, off_route_m: float) -> Poi:
    return Poi(
        id=f"poi-{name}",
        name=name,
        category=PoiCategory.VIEWPOINT,
        coordinate=north(at_m, east=off_route_m),
        source=PoiSource.PLACES,
        place_id=f"ChIJ-{name}",
        score=score,
    )
...
class TestTheFloor:
    def test_a_place_the_judge_recommended_is_taken(self):
        assert names(worth_routing_through([poi("lookout", 0.9)], legs=[leg()])) == ["lookout"]

    def test_a_place_the_judge_hedged_about_is_left_for_the_rider(self):
        assert worth_routing_through([poi("meh", MIN_SCORE - 0.01)], legs=[leg()]) == ()

    def test_the_floor_itself_is_included_not_excluded(self):
        assert names(worth_routing_through([poi("edge", MIN_SCORE)], legs=[leg()])) == ["edge"]
    ...
    def test_a_place_nobody_judged_is_never_added_automatically(self):
        ...
        assert worth_routing_through([poi("hand-dropped", None)], legs=[leg()]) == ()
...
class TestTheCount:
    def test_it_takes_the_best_first(self):
        ...
        assert names(worth_routing_through(found, legs=[leg()], limit=1)) == ["great"]

    def test_it_stops_at_the_limit_it_was_given(self):
        found = [poi(f"p{index}", 0.9, along=index / 10) for index in range(9)]
        assert len(worth_routing_through(found, legs=[leg()], limit=3)) == 3

    def test_asking_for_none_adds_none(self):
        assert worth_routing_through([poi("great", 0.95)], legs=[leg()], limit=0) == ()
    ...
    def test_the_default_applies_when_no_limit_is_asked_for(self):
        found = [poi(f"p{index}", 0.9, along=index / 10) for index in range(9)]
        assert len(worth_routing_through(found, legs=[leg()])) == default_limit([leg()])
...
class TestTheDetourBudget:
    def test_a_place_beside_the_road_costs_almost_nothing(self):
        ...
        assert names(worth_routing_through(found, legs=[leg()])) == ["roadside"]
    ...
    def test_a_place_that_would_reshape_the_trip_is_refused(self):
        ...
        assert worth_routing_through(found, legs=[leg(400_000.0)]) == ()
    ...
    def test_the_budget_is_spent_across_all_the_additions_not_each_one(self):
        ...
        assert len(worth_routing_through(found, legs=[leg(400_000.0)], limit=3)) == 2
    ...
    def test_it_keeps_looking_after_one_place_proves_too_expensive(self):
        ...
        assert names(worth_routing_through(found, legs=[leg(400_000.0)], limit=2)) == ["near"]
    ...
    def test_the_cost_over_states_an_out_and_back(self):
        ...
        assert worth_routing_through([poi("far", 0.9, off_route_m=just_over)], legs=[leg()]) == ()
    ...
    def test_a_leg_with_no_length_admits_nothing_rather_than_everything(self):
        ...
        assert worth_routing_through([poi("anything", 0.99)], legs=[empty]) == ()


class TestATripIsSeveralLegs:
    """Both bounds read one leg, and on a four-leg trip that is the wrong leg.

    Measured on Tim's 797 km trip: the count came out 2, from the longest leg's 5.1 hours,
    where the trip is 14.5 hours and should give 7. The budget was 15% of the longest leg —
    40.8 km — governing insertions into all four, which is too generous beside a 139 km leg
    and too mean beside a 272 km one.
    """

    def test_the_count_is_paced_by_the_whole_ride(self):
        four = [span(index * 200_000.0, (index + 1) * 200_000.0) for index in range(4)]
        assert default_limit(four) == 4 * default_limit(four[:1])

    def test_a_place_beside_a_short_leg_cannot_spend_the_long_leg_s_allowance(self):
        long_, short = span(0.0, 400_000.0), span(400_000.0, 440_000.0)
        budget_of_short = DETOUR_BUDGET_FRACTION * 40_000.0
        too_far = beside("by the connector", 0.9, at_m=420_000.0, off_route_m=budget_of_short)
        assert worth_routing_through([too_far], legs=[long_, short], limit=2) == ()

    def test_the_same_place_would_fit_if_it_sat_beside_the_long_leg(self):
        """Same detour, different leg: the bound is local, and this is what makes it local."""
        long_, short = span(0.0, 400_000.0), span(400_000.0, 440_000.0)
        budget_of_short = DETOUR_BUDGET_FRACTION * 40_000.0
        near_long = beside("by the long leg", 0.9, at_m=200_000.0, off_route_m=budget_of_short)
        assert names(worth_routing_through([near_long], legs=[long_, short], limit=2)) == [
            "by the long leg"
        ]

    def test_spending_one_leg_s_budget_does_not_starve_another(self):
        first, second = span(0.0, 400_000.0), span(400_000.0, 800_000.0)
        expensive = beside("expensive", 0.95, at_m=200_000.0, off_route_m=29_000.0)
        cheap = beside("cheap", 0.90, at_m=600_000.0, off_route_m=100.0)
        chosen = names(worth_routing_through([expensive, cheap], legs=[first, second], limit=2))
        assert chosen == ["expensive", "cheap"]

    def test_a_trip_with_no_routed_legs_adds_nothing(self):
        assert worth_routing_through([poi("great", 0.95)], legs=[]) == ()

    def test_no_legs_still_paces_at_least_one_stop(self):
        """`default_limit` must not divide an empty trip into zero stops."""
...
class TestWhatItLeavesAlone:
    def test_a_place_already_on_the_route_is_not_added_twice(self):
        ...
        assert names(worth_routing_through(found, legs=[leg()], limit=2)) == ["new"]

    def test_nothing_found_is_nothing_added(self):
        assert worth_routing_through([], legs=[leg()]) == ()

    def test_it_returns_them_best_first(self):
        found = [poi("second", 0.8, along=0.2), poi("first", 0.95, along=0.8)]
        assert names(worth_routing_through(found, legs=[leg()], limit=2)) == ["first", "second"]

    @pytest.mark.parametrize("limit", [-1, -10])
    def test_a_nonsense_limit_adds_nothing_rather_than_everything(self, limit):
        assert worth_routing_through([poi("great", 0.95)], legs=[leg()], limit=limit) == ()
